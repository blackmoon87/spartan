    ...
    try {
        ...
        if ($userCount === 0) {
            ...
            if ($adminId && $authorId) {
                ...
                $stmtPost = $app->db->prepare("INSERT INTO posts (user_id, title, slug, body, cover_image, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmtPost->execute([(int)$authorId, 'Welcome to Spartan Blogger', 'welcome-to-spartan-blogger', 'This is the very first blog post on this amazing Spartan framework.', null, $now, $now]);
            }
        }
    } catch (\Throwable $e) {
